...
interface E2EEState {
  ...
  // Async actions
  // Note: the master key is stored in sessionStorage (cleared on tab close —
  // see crypto.ts). `ttlDays` is retained for caller-compat but ignored:
  // sessionStorage has no TTL.
...
export const useE2EEStore = create<E2EEState>((set, get) => ({
  ...
  // Store master key in sessionStorage (cleared on tab close — see crypto.ts).
  // ttlDays is accepted but ignored — sessionStorage has no TTL.
